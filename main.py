...
from src import CurrencyApiClient, RatesPDFBuilder, RatesStorage
...
DATES = [datetime.date.today() - datetime.timedelta(days=days) for days in range(1, int(environ.get('INTERVAL_DAYS')))]
...
async def main():
	...
	pdf_builder.compose()
	logging.info(f'Rates parsed and compiled into plots to PDF: [{PDF_FILE_PATH}]')
